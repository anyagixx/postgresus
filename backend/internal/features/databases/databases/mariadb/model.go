...
func (m *MariadbDatabase) CreateReadOnlyUser(
	...
	for attempt := range maxRetries {
		...
		newUsername := fmt.Sprintf("maria-%s", uuid.New().String()[:8])
...
// 2. Grants SELECT, SHOW VIEW, LOCK TABLES, TRIGGER, EVENT privileges on the database
// 3. Grants PROCESS privilege on all databases (needed for backup operations)
